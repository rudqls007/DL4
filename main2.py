...
warnings.filterwarnings('ignore')무
...
print(accuracy_score(test_outputs, y_val))  # 제현율


# 신경망에서 필요한 모든 파라미터를 무작위로 선택했다는 감안할 때 74% 정확도는 나쁘지 않다.
# 파라미터( 훈련 / 테스트 데이터 셋 분할, 은닉층 개수 및 크기 등 ) 변경하면서 더 나은 성능을 찾아보는 것도 의미가 있다.