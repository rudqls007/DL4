    ...
    1. 인공 신경망
        ...
        2) perceptron
            - 뉴런의 원리를 본떠 만든 인공 구조
                - 입력값과 가중치를 곱함
                - 곱한 값들의 총 합을 구함
                - 총 합이 0을 넘으면  1, 넘지 않으면 0을 출력
            - 입력값, 가중치, 활성화 함수로 이루어진 간단한 구조
                - 원하는 출력값을 내보내도록 가중치를 조정해가는 작업 => 학습, 훈련 (training)
                - 모델을 훈련시켜 최적인 가중치를 구함
            - 단순 퍼셉트론은 선형 분류 문제만 해결한다는 한게가 있다.
                - 입력값과 가중치의 선형 결합으로 이루어진 구조
                - 퍼셉트론 하나로는 비선형 분류 문제를 해결 안됨
                    => 다중 퍼셉트론 (multi-layer perceptron)을 만들면 해결된다.
        ...
        5) 경사 하강법
            ...
            - 경사 하강법 ( gradient descent )
                ...
                - 절차
                    ...
                    - 현재 위치의 기울기가 0이 될 떄까지 1~2단계를 반복함
                        - 손실 함수가 최소가 될 때까지 반복
                    - 학습률(learning rage)
                        - 기울기 방향으로 얼마만큼 이동할 지 결정하는 값
                        - 가중치를 한번에 얼마나 갱신할 지 결정
                        - 다음 가중치
                            - 기존 가중치에서 '학습률과 기울기(미분값)를 곱한 값' 을 뺀 값
                        - 학습률에 따라 훈련 속도가 달라짐
                            - 한 걸음에 얼마만큼 이동할 지 결정하는게 학습률이다.
                                - 학습률이 너무 크면 훈련 속도는 빨라짐,
                                - 최적 가중치를 찾지 못할 수도 있다.
                                - 학습률이 너무 작으면 훈련 속도가 느려짐.
                            - 하이퍼파라미터이기에 우리가 직접 설정해줘야함.
                                - 보통 0.1 ~ 0.0001 범위의 값을 사용함
                    - mini batch
                        - 데이터를 미니배치 단위, 무작위로 추출해 경사 하강법을 수행
                            - 여러 데이터의 묶음
                            - 데이터를 하나씩 훈련하기보다는 여러 개를 한 묶음으로 처리함 ( 효율적이다. )
                ...
                6) 순전파 & 역전파
                    ...
                    - 순전파와 역전파를 반복하며 최적 가중치를 찾아 신경망 학습이 이루어짐.

            2. 딥러닝 알고리즘 성능 향상
                1) 드롭아웃(dropout)
                    - 과대적합을 방지하기 위하여 신경망 훈련 과정에서0 무작위로 일불 뉴런을 제외하는 기법
                    - 얼마나 많은 뉴런을 드롭아웃할지는 하이퍼파라미터로 설정할 수 있다.
